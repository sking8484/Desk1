...
deployLambdas: publish
	for service in `cat $${SERVICE_LIST}`; do \
		...
		ALPACAPUBKEY=${ALPACAPUBKEY} ALPACAPRIVKEY=${ALPACAPRIVKEY} Service=$${service} ENV=${ENV} MAINPERFTABLE=${MAINPERFTABLE} MAINFACTORTABLE=${MAINFACTORTABLE} MAINSTATSTABLE=${MAINSTATSTABLE};\
	done
